...
- 訂閱制（完整版 1,000/月・2,700/季；陽春版 300/月＋每週上限 7 筆）→ 見 `docs/訂閱制.md`（含定價分析、n8n 建置規格、額度控管作法）。前端訂閱公告已上線（`報刀_v2.html` 的 `SUB_NOTICE`），後端 `ack-subscription`／`google-login` 回傳訂閱方案／`manual-baodao` 額度控管**尚未建置**。
