...
const BASE_URL = 'https://a29052908.github.io/YD-Jiang/CRM';
...
async function main() {
  ...
  try {
    ...
    // 3. 截圖客戶總表 (尚無客戶)
    console.log('📍 步驟 3: 截圖客戶清單 (初始空狀態)');
    await page.screenshot({ path: `${SCREENSHOT_DIR}/02-client-list-empty.png`, fullPage: true });
    console.log('✅ 已截圖: 02-client-list-empty.png');
    ...
    for (let i = 0; i < testClients.length; i++) {
      ...
      if (i < testClients.length - 1) {
        await page.goto(`${BASE_URL}/crm-list.html`);
        await page.waitForLoadState('networkidle');
      }
    }
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/03-client-list-populated.png`, fullPage: true });
    console.log('✅ 已截圖: 03-client-list-populated.png');

    // 7. 測試篩選功能
    console.log('\n📍 步驟 7: 測試篩選功能 - 按階段 "簡報"');
    await page.click('div.stat-chip[data-filter="簡報"]');
    await page.waitForTimeout(500);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/04-client-list-filter-stage.png`, fullPage: true });
    console.log('✅ 已截圖: 04-client-list-filter-stage.png');

    // 8. 重置篩選
    await page.click('div.stat-chip.active[data-filter="all"]');
    await page.waitForTimeout(500);

    // 9. 點開第一個客戶 (測試客A - 簡報階段)
    console.log('\n📍 步驟 8: 打開單一客戶檔案');
    const firstClientCard = await page.locator('.client-card').first();
    ...
    // 11. 尋找並點擊 AI 建議按鈕 (如果存在)
    console.log('\n📍 步驟 10: 檢查 AI 建議功能');
    const aiButton = await page.locator('button:has-text("AI 建議")').first();
    if (await aiButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      console.log('📍 找到 AI 建議按鈕，點擊並等待結果');
      await aiButton.click();
      await page.waitForTimeout(3000); // 等待 API 回應
      await page.screenshot({ path: `${SCREENSHOT_DIR}/06-ai-suggestion-result.png`, fullPage: true });
      console.log('✅ 已截圖: 06-ai-suggestion-result.png');
    } else {
      console.log('⚠️  AI 建議功能尚未實裝前端，跳過此步驟');
    }
    ...
    // 找第二個客戶 (成交)，點開編輯
    const clientCards = await page.locator('.client-card').all();
    if (clientCards.length >= 4) {
      await clientCards[3].click(); // 測試客D (成交)
      await page.waitForLoadState('networkidle');

      // 檢查是否有編輯按鈕
      const editButton = await page.locator('button:has-text("編輯")').first();
      if (await editButton.isVisible({ timeout: 1000 }).catch(() => false)) {
        await editButton.click();
        await page.waitForLoadState('networkidle');
        await page.screenshot({ path: `${SCREENSHOT_DIR}/07-edit-form.png`, fullPage: true });
        console.log('✅ 已截圖: 07-edit-form.png');
      } else {
        console.log('⚠️  編輯功能尚未實裝，跳過此步驟');
      }
    }
    ...
    await page.click('button.identity-btn.yd');
    await page.waitForLoadState('networkidle');
    await page.screenshot({ path: `${SCREENSHOT_DIR}/09-mobile-client-list.png`, fullPage: true });
    console.log('✅ 已截圖: 09-mobile-client-list.png');
